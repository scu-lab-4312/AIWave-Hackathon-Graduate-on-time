"""Real AWS transport for invoking a Taxi AgentCore Runtime."""
...
class AgentCoreTaxiClient(SpecialistClient):
    def __init__(self, runtime_arn: str, region: str = "us-west-2", qualifier: str = "DEFAULT"):
        if not runtime_arn:
            raise ValueError("TAXI_AGENT_RUNTIME_ARN is required")
    ...
    def invoke(self, request: HandoffRequest) -> SpecialistResponse:
        ...
        try:
            ...
            raise RuntimeError(f"Taxi Agent returned an invalid contract: {error}") from error
