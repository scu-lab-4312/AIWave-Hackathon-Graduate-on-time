...
這個 Agent 的範圍刻意很小：只依 `city`、`district` 查詢附近的虛擬藥局，回傳至少三間藥局的藥師姓名與虛擬 LINE ID，之後由使用者自行聯絡藥師。
...
services/medical_ops/
  lambda_function.py   # RDS lookup + demo seed
  schema.sql           # medical_pharmacies only
